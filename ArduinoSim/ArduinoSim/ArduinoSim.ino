...
void loop()
{
  if (Serial.available() > 0) {
    ...
    //Serial.println(String((byte)(inByte), HEX));
/*    delay(10);      //Тест КС
    byte buf[] = {inByte};
    unsigned short crc = calcCrc(buf, 1);
    Serial.print("Recieved: 0x");
    Serial.println(String((byte)(inByte), HEX));
...
unsigned short calcCrc(byte * data, int dataLen)
{
 unsigned short crc = 0x0000;
 while(dataLen--)
 {
   byte index = (byte)((crc >> 8) ^ *data++);
   crc = (unsigned short)(crc_table[index] ^ (crc << 8));
 }
 return crc;
}
